...
A Haystack DocumentStore backed by FalkorDB â€” a high-performance graph database.
...
**flat** at the same level as `id` and `content` â€” exactly the same layout as
the `neo4j-haystack` reference integration.

Vector search is performed via FalkorDB's native vector index â€”
...
- **verify_connectivity** (<code>bool</code>) – When `True` a connectivity probe is run
  immediately in `__init__` â€” raises if the server is unreachable.
...
`content` â€” no prefix is added. This matches the layout used by the
