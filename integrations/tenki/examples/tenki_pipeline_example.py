...
"""
Example: Haystack Pipeline with an Agent and Tenki sandbox tools.

Demonstrates that a Pipeline containing an Agent with TenkiToolset can be:
...
share a single TenkiSandbox after the round-trip, so the agent operates
in one live sandbox environment.

Requirements:
    pip install tenki-haystack openai

Environment variables:
    TENKI_AUTH_TOKEN - your Tenki auth token (or TENKI_API_KEY)
...
def build_pipeline() -> Pipeline:
    agent = Agent(
        ...
        tools=TenkiToolset(name="haystack", cpu_cores=1, memory_mb=1024),
        system_prompt=(
...
def verify_roundtrip(original: Pipeline, restored: Pipeline) -> None:
    ...
    orig_ts: TenkiToolset = orig_agent.tools  # type: ignore[assignment]
    rest_ts: TenkiToolset = rest_agent.tools  # type: ignore[assignment]

    assert type(rest_ts).__name__ == "TenkiToolset", "Toolset type mismatch"
    assert [t.name for t in rest_ts] == [t.name for t in orig_ts], "Tool names mismatch"
    assert rest_ts.sandbox.name == orig_ts.sandbox.name
    assert rest_ts.sandbox.cpu_cores == orig_ts.sandbox.cpu_cores
    assert rest_ts.sandbox.memory_mb == orig_ts.sandbox.memory_mb

    sandbox_ids = {id(t._tenki_sandbox) for t in rest_ts}
