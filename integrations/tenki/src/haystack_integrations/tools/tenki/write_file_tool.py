...
class WriteFileTool(Tool):
    """
    A :class:`~haystack.tools.Tool` that writes files to a Tenki sandbox filesystem.
    ...
    ```python
    from haystack_integrations.tools.tenki import TenkiSandbox, WriteFileTool
    ...
    def __init__(self, sandbox: TenkiSandbox) -> None:
        ...
        :param sandbox: The :class:`TenkiSandbox` instance to write files to.
        """

        def write_file(path: str, content: str) -> str:
            sb = sandbox._require_sandbox()
            try:
                sb.fs.write_text(path, content)
