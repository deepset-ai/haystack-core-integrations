...
from haystack_integrations.components.generators.hpc_ai.chat.chat_generator import (
    HPCAIChatGenerator,
)
...
def mock_async_chat_completion():
    """
    Mock the async HPC-AI API completion response and reuse it for async tests.
    """
    with patch(
...
class TestHPCAIChatGeneratorAsync:
    def test_init_default_async(self, monkeypatch):
        ...
        assert component.async_client.base_url == "https://api.hpc-ai.com/inference/v1/"
    ...
    async def test_run_async_with_params(self, chat_messages, mock_async_chat_completion, monkeypatch):
        monkeypatch.setenv("HPC_AI_API_KEY", "fake-api-key")
        component = HPCAIChatGenerator(
            model="moonshotai/kimi-k2.5", generation_kwargs={"max_tokens": 10, "temperature": 0.5}
        )
        response = await component.run_async(chat_messages)
    ...
    async def test_live_run_with_tools_streaming_async(self, tools):
        ...
        component = HPCAIChatGenerator(tools=tools, streaming_callback=callback, model="moonshotai/kimi-k2.5")
