...
from haystack_integrations.components.generators.hpc_ai.chat.chat_generator import HPCAIChatGenerator
...
def mock_chat_completion():
    """
    Mock the HPC-AI API completion response and reuse it for tests
    """
...
class TestHPCAIChatGenerator:
    def test_init_default(self, monkeypatch):
        monkeypatch.setenv("HPC_AI_API_KEY", "test-api-key")
        component = HPCAIChatGenerator()
        assert component.client.api_key == "test-api-key"
        assert component.model == "minimax/minimax-m2.5"
        assert component.api_base_url == "https://api.hpc-ai.com/inference/v1"
        assert component.streaming_callback is None
        assert not component.generation_kwargs

    def test_init_default_uses_base_url_env(self, monkeypatch):
        monkeypatch.setenv("HPC_AI_API_KEY", "test-api-key")
        monkeypatch.setenv("HPC_AI_BASE_URL", "https://custom.hpc-ai.test/v1")

        component = HPCAIChatGenerator()

        assert component.api_base_url == "https://custom.hpc-ai.test/v1"

    def test_supported_models(self):
        assert HPCAIChatGenerator.SUPPORTED_MODELS == [
            "minimax/minimax-m2.5",
            "moonshotai/kimi-k2.5",
        ]

    def test_init_fail_wo_api_key(self, monkeypatch):
        monkeypatch.delenv("HPC_AI_API_KEY", raising=False)
    ...
    def test_to_dict_default(self, monkeypatch):
        ...
        assert (
            data["type"] == "haystack_integrations.components.generators.hpc_ai.chat.chat_generator.HPCAIChatGenerator"
        )

        expected_params = {
            "api_key": {"env_vars": ["HPC_AI_API_KEY"], "strict": True, "type": "env_var"},
            "model": "minimax/minimax-m2.5",
    ...
    def test_from_dict(self, monkeypatch):
        monkeypatch.setenv("HPC_AI_API_KEY", "fake-api-key")
        data = {
            "type": ("haystack_integrations.components.generators.hpc_ai.chat.chat_generator.HPCAIChatGenerator"),
            "init_parameters": {
                "api_key": {"env_vars": ["HPC_AI_API_KEY"], "strict": True, "type": "env_var"},
                "model": "minimax/minimax-m2.5",
                "api_base_url": "test-base-url",
        ...
        component = HPCAIChatGenerator.from_dict(data)
        assert component.model == "minimax/minimax-m2.5"
        assert component.streaming_callback is print_streaming_chunk
        assert component.api_base_url == "test-base-url"
        assert component.generation_kwargs == {"max_tokens": 10, "some_test_param": "test-params"}
        assert component.api_key == Secret.from_env_var("HPC_AI_API_KEY")
    ...
    def test_from_dict_fail_wo_env_var(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match=r"None of the .* environment variables are set"):
            HPCAIChatGenerator.from_dict(data)
    ...
    def test_run_with_params(self, chat_messages, mock_chat_completion, monkeypatch):
        monkeypatch.setenv("HPC_AI_API_KEY", "fake-api-key")
        component = HPCAIChatGenerator(generation_kwargs={"max_tokens": 10, "temperature": 0.5})
        response = component.run(chat_messages)

        # check that the component calls the HPC-AI API with the correct parameters
        # for HPC-AI, these are passed in the extra_body parameter
    ...
    @pytest.mark.skipif(
        not os.environ.get("HPC_AI_API_KEY", None),
        reason="Export an env var called HPC_AI_API_KEY containing the HPC-AI API key to run this test.",
    )
    @pytest.mark.integration
    def test_live_run(self):
        chat_messages = [ChatMessage.from_user("What's the capital of France")]
        component = HPCAIChatGenerator(model="moonshotai/kimi-k2.5")
        results = component.run(chat_messages)
        assert len(results["replies"]) == 1
        message: ChatMessage = results["replies"][0]
        assert message.text
        assert "Paris" in message.text
        assert "kimi-k2.5" in message.meta["model"]
    ...
    def test_live_run_wrong_model(self, chat_messages):
        component = HPCAIChatGenerator(model="something-obviously-wrong")
    ...
    def test_live_run_streaming(self):
        ...
        callback = Callback()
        component = HPCAIChatGenerator(streaming_callback=callback, model="moonshotai/kimi-k2.5")
    ...
    def test_live_run_with_tools(self, tools):
        ...
        component = HPCAIChatGenerator(model="moonshotai/kimi-k2.5", tools=tools)
        results = component.run(chat_messages)
        assert len(results["replies"]) == 1
    ...
    def test_live_run_with_tools_and_response(self, tools):
        """
        Integration test that the HPCAIChatGenerator component can run with tools and get a response.
        """
        initial_messages = [ChatMessage.from_user("What's the weather like in Paris and Berlin?")]
        component = HPCAIChatGenerator(tools=tools, model="moonshotai/kimi-k2.5")
    ...
    def test_live_run_with_tools_streaming(self, tools):
        """
        Integration test that the HPCAIChatGenerator component can run with tools and streaming.
        """
        component = HPCAIChatGenerator(
            tools=tools, streaming_callback=print_streaming_chunk, model="moonshotai/kimi-k2.5"
        )
    ...
    @pytest.mark.integration
    def test_pipeline_with_hpc_ai_chat_generator(self, tools):
        """
        Test that the HPCAIChatGenerator component can be used in a pipeline
        """
        pipeline = Pipeline()
        pipeline.add_component("generator", HPCAIChatGenerator(tools=tools, model="moonshotai/kimi-k2.5"))
    ...
    def test_serde_in_pipeline(self, monkeypatch):
        """
        Test serialization/deserialization of HPCAIChatGenerator in a Pipeline,
        including YAML conversion and detailed dictionary validation
        """
        # Set mock API key
        monkeypatch.setenv("HPC_AI_API_KEY", "test-key")
        ...
        expected_dict = {
            ...
            "components": {
                "generator": {
                    "type": "haystack_integrations.components.generators.hpc_ai.chat.chat_generator.HPCAIChatGenerator",
                    "init_parameters": {
                        "api_key": {"type": "env_var", "env_vars": ["HPC_AI_API_KEY"], "strict": True},
                        "model": "minimax/minimax-m2.5",
                        "streaming_callback": "haystack.components.generators.utils.print_streaming_chunk",
                        "api_base_url": "https://api.hpc-ai.com/inference/v1",
                        "generation_kwargs": {"temperature": 0.7},
                        "tools": [
                            {
                                ...
                                "data": {
                                    ...
                                    "description": "useful to determine the weather in a given location",
                                    "parameters": {"city": {"type": "string"}},
                                    "function": "tests.test_hpc_ai_chat_generator.weather",
                                },
                            }
                        ],
                        "http_client_kwargs": None,
...
class TestChatCompletionChunkConversion:
    def test_handle_stream_response(self):
        ...
        llm = HPCAIChatGenerator(api_key=Secret.from_token("test-api-key"))
        result = llm._handle_stream_response(hpc_ai_chunks, callback=collector_callback)[0]  # type: ignore
        ...
        assert result.meta["model"] == "minimax/minimax-m2.5"
