...
def opentelemetry_tracer(span_exporter: InMemorySpanExporter) -> opentelemetry.trace.Tracer:
    ...
    return trace_provider.get_tracer("my_test")
...
class TestOpenTelemetryTracer:
    ...
    def test_current_span_returns_none_without_active_span(
        self, opentelemetry_tracer: opentelemetry.trace.Tracer
    ) -> None:
        tracer = OpenTelemetryTracer(opentelemetry_tracer)
        # Outside of any `trace` context there is no recording span, so `current_span` should return None.
