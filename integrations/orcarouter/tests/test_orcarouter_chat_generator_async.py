...
from haystack_integrations.components.generators.orcarouter.chat.chat_generator import (
...
class TestOrcaRouterChatGeneratorAsync:
    ...
    async def test_run_async_with_params(self, chat_messages, mock_async_chat_completion, monkeypatch):
        monkeypatch.setenv("ORCAROUTER_API_KEY", "fake-api-key")
        component = OrcaRouterChatGenerator(
            model="openai/gpt-4o-mini", generation_kwargs={"max_tokens": 10, "temperature": 0.5}
        )
        response = await component.run_async(chat_messages)
    ...
    async def test_live_run_with_tools_streaming_async(self, tools):
        ...
        component = OrcaRouterChatGenerator(tools=tools, streaming_callback=callback, model="openai/gpt-4o-mini")
