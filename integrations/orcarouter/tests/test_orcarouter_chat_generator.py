...
from haystack_integrations.components.generators.orcarouter.chat.chat_generator import OrcaRouterChatGenerator
...
def mock_chat_completion():
    """
    Mock the OrcaRouter API completion response and reuse it for tests
    """
...
class TestOrcaRouterChatGenerator:
    ...
    def test_run_with_params(self, chat_messages, mock_chat_completion, monkeypatch):
        monkeypatch.setenv("ORCAROUTER_API_KEY", "fake-api-key")
        component = OrcaRouterChatGenerator(generation_kwargs={"max_tokens": 10, "temperature": 0.5})
        response = component.run(chat_messages)
    ...
    @pytest.mark.skipif(
        not os.environ.get("ORCAROUTER_API_KEY", None),
    ...
    def test_live_run(self):
        ...
        component = OrcaRouterChatGenerator(model="openai/gpt-4o-mini")
    ...
    def test_live_run_wrong_model(self, chat_messages):
        # OrcaRouter returns a 503 for unknown models, which the OpenAI SDK would otherwise retry; disable retries.
        component = OrcaRouterChatGenerator(model="something-obviously-wrong", max_retries=0)
    ...
    def test_live_run_streaming(self):
        ...
        callback = Callback()
        component = OrcaRouterChatGenerator(streaming_callback=callback, model="openai/gpt-4o-mini")
    ...
    def test_live_run_with_tools(self, tools):
        ...
        component = OrcaRouterChatGenerator(model="openai/gpt-4o-mini", tools=tools)
        results = component.run(chat_messages)
        assert len(results["replies"]) == 1
    ...
    def test_live_run_with_tools_and_response(self, tools):
        """
        Integration test that the OrcaRouterChatGenerator component can run with tools and get a response.
        """
        initial_messages = [ChatMessage.from_user("What's the weather like in Paris and Berlin?")]
        component = OrcaRouterChatGenerator(tools=tools, model="openai/gpt-4o-mini")
    ...
    def test_live_run_with_tools_streaming(self, tools):
        """
        Integration test that the OrcaRouterChatGenerator component can run with tools and streaming.
        """
        component = OrcaRouterChatGenerator(
            tools=tools, streaming_callback=print_streaming_chunk, model="openai/gpt-4o-mini"
        )
    ...
    def test_pipeline_with_orcarouter_chat_generator(self, tools):
        ...
        """
        pipeline = Pipeline()
        pipeline.add_component("generator", OrcaRouterChatGenerator(tools=tools, model="openai/gpt-4o-mini"))
    ...
    def test_serde_in_pipeline(self, monkeypatch):
        """
        Test serialization/deserialization of OrcaRouterChatGenerator in a Pipeline,
        ...
        expected_dict = {
            ...
            "components": {
                "generator": {
                    "type": "haystack_integrations.components.generators.orcarouter.chat.chat_generator.OrcaRouterChatGenerator",  # noqa: E501
                    "init_parameters": {
                        "api_key": {"type": "env_var", "env_vars": ["ORCAROUTER_API_KEY"], "strict": True},
                        "model": "openai/gpt-4o-mini",
                        "streaming_callback": "haystack.components.generators.utils.print_streaming_chunk",
                        "api_base_url": "https://api.orcarouter.ai/v1",
                        "generation_kwargs": {"temperature": 0.7},
...
class TestChatCompletionChunkConversion:
    def test_handle_stream_response(self):
        ...
        llm = OrcaRouterChatGenerator(api_key=Secret.from_token("test-api-key"))
        result = llm._handle_stream_response(orcarouter_chunks, callback=collector_callback)[0]  # type: ignore
