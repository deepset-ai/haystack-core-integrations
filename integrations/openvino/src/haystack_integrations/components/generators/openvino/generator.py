...
class OpenVINOGenerator(HuggingFaceLocalGenerator):
    """
    Generator based on a OpenVINO model.

    This component provides an interface to generate text using a OpenVINO model that runs locally.

    Usage example:
    ```python
    from haystack.components.generators import OpenVINOGenerator
