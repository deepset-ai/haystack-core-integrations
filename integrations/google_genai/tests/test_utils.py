...
def test_get_client_vertex_no_credentials_raises(monkeypatch):
    ...
    with pytest.raises(ValueError):
        _get_client(api_key=api_key, use_vertex_ai=True, vertex_ai_project=None, vertex_ai_location=None)
...
def test_get_client_vertex_project_and_location(monkeypatch):
    ...
    with patch("haystack_integrations.components.common.google_genai.utils.Client") as mock_client:
        client = _get_client(
            api_key=api_key, use_vertex_ai=True, vertex_ai_project="test-project", vertex_ai_location="test-location"
        )
...
def test_get_client_vertex_api_key(monkeypatch):
    ...
    with patch("haystack_integrations.components.common.google_genai.utils.Client") as mock_client:
        client = _get_client(api_key=api_key, use_vertex_ai=True, vertex_ai_project=None, vertex_ai_location=None)
...
def test_get_client_gemini_api_key(monkeypatch):
    ...
    with patch("haystack_integrations.components.common.google_genai.utils.Client") as mock_client:
        client = _get_client(api_key=api_key, use_vertex_ai=False, vertex_ai_project=None, vertex_ai_location=None)
...
def test_get_client_gemini_api_key_no_env_var_raises(monkeypatch):
    ...
    api_key = Secret.from_env_var("GEMINI_API_KEY", strict=False)
    with pytest.raises(ValueError):
        _get_client(api_key=api_key, use_vertex_ai=False, vertex_ai_project=None, vertex_ai_location=None)
