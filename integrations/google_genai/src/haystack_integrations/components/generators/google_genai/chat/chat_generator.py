...
from haystack.dataclasses import StreamingCallbackT, StreamingChunk, ToolCall
...
def _convert_message_to_google_genai_format(message: ChatMessage) -> types.Content:
    ...
    if message.tool_calls:
        for tool_call in message.tool_calls:
            parts.append(
                types.Part(function_call=types.FunctionCall(id=tool_call.id, name=tool_call.tool_name, args=tool_call.arguments))
            )
            ...
            parts.append(
                types.Part(
                    function_response=types.FunctionResponse(
                        id=result.origin.id,
                        name=result.origin.tool_name, response={"result": result.result}
                    )
                )
            )
