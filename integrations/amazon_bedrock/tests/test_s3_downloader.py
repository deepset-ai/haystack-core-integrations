...
# Integration tests - these require real AWS credentials and will be skipped in CI if authentication fails
@pytest.mark.integration
class TestS3DownloaderIntegration:
    """Integration tests for S3Downloader that require real AWS credentials."""

    def test_warm_up_real_credentials(self):
        """Test warm up with real AWS credentials."""
        # Use real credentials from environment
        downloader = S3Downloader()
        
        # This should not raise an exception if credentials are valid
        # If credentials are invalid, it will log a warning but not fail
        downloader.warm_up()

    def test_download_from_public_s3_bucket(self):
        """Test downloading from a publicly accessible S3 bucket."""
        # Use a well-known public S3 bucket for testing
        # Note: This test might fail if the public bucket becomes unavailable
        downloader = S3Downloader()
        
        # AWS S3 documentation often uses public buckets for examples
        # This is a common pattern in integration tests
        public_url = "s3://haystack-integration-test-bucket/test-file.txt"
        
        try:
            result = downloader.run(public_url)
            ...
            assert metadata["backend"] == "s3"
            assert metadata["s3_bucket"] == "haystack-integration-test-bucket"
            assert metadata["s3_key"] == "test-file.txt"
            assert metadata["source_url"] == public_url
            assert "download_time" in metadata
            assert "checksum" in metadata
            
        except FileNotFoundError:
            # Skip test if the test bucket/file doesn't exist
            pytest.skip("Public test bucket or file not available")
        except Exception as e:
            # If it's a credentials issue, skip the test
            if "credentials" in str(e).lower() or "access" in str(e).lower():
                pytest.skip(f"AWS credentials issue: {e}")
            else:
                raise

    def test_download_nonexistent_file_real_aws(self):
        """Test downloading a nonexistent file from a real S3 bucket."""
        downloader = S3Downloader()
        
        # Try to download from a bucket that likely exists but with a nonexistent key
        nonexistent_url = "s3://haystack-integration-test-bucket/nonexistent-file-12345.txt"
        
        try:
            with pytest.raises(FileNotFoundError, match="File not found in S3"):
                downloader.run(nonexistent_url)
        except Exception as e:
            # If it's a credentials or bucket access issue, skip the test
            if any(keyword in str(e).lower() for keyword in ["credentials", "access", "forbidden", "bucket"]):
                pytest.skip(f"AWS access issue: {e}")
            else:
                raise

    def test_download_from_nonexistent_bucket_real_aws(self):
        """Test downloading from a nonexistent S3 bucket."""
        downloader = S3Downloader()
        
        # Use a bucket name that's very unlikely to exist
        nonexistent_bucket_url = "s3://haystack-nonexistent-bucket-12345678/file.txt"
        
        try:
            with pytest.raises(FileNotFoundError, match="Bucket not found"):
                downloader.run(nonexistent_bucket_url)
        except Exception as e:
            # If it's a credentials issue, skip the test
    ...
    def test_custom_endpoint_integration(self):
        """Test S3Downloader with custom endpoint (e.g., MinIO or other S3-compatible service)."""
        # This test can be used with MinIO or other S3-compatible services
        # Skip if no custom endpoint is configured
        import os
        custom_endpoint = os.getenv("S3_CUSTOM_ENDPOINT")
        
        if not custom_endpoint:
            pytest.skip("S3_CUSTOM_ENDPOINT environment variable not set")
        
        downloader = S3Downloader(
            endpoint_url=custom_endpoint,
            verify_ssl=False  # Often needed for local MinIO instances
        )
        
        # Test warm up with custom endpoint
        downloader.warm_up()

    def test_regional_bucket_access(self):
        """Test accessing S3 bucket from a specific region."""
        downloader = S3Downloader(aws_region="us-west-2")
        
        try:
            # Test warm up with specific region
            downloader.warm_up()
        except Exception as e:
            if "credentials" in str(e).lower():
                pytest.skip(f"AWS credentials issue: {e}")
            else:
                # Just verify the component was configured correctly
                assert downloader.aws_region == "us-west-2" 