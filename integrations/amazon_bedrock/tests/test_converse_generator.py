...
def test_tool_usage():
        ...
        "output": {
            "message": {
                ...
                "content": [
                    ...
                    {
                        "toolUse": {
                            "toolUseId": "tooluse_-Tp78_OeSq-1DSsP0B__TA",
                    ...
                    {
                        "toolUse": {
                            "toolUseId": "tooluse_gdYvqeiGTme7toWoV4sSKw",
    ...
    assert json.dumps(result["message"].content.content[1].input) == """{"location": "Paris", "unit": "celsius"}"""
    assert json.dumps(result["message"].content.content[2].input) == """{"timezone": "America/New_York"}"""
    assert result["message"].content.content[1].input["location"] == "Paris"
    assert result["message"].content.content[1].input["unit"] == "celsius"
    assert result["message"].content.content[2].input["timezone"] == "America/New_York"
