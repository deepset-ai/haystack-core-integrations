...
from haystack_integrations.components.websearch.serpbase import SerpBaseError, SerpBaseWebSearch

EXAMPLE_SERPBASE_RESPONSE = {
    "status": 0,
    ...
    "featured_snippet": {
        "title": "Olivia Wilde and Harry Styles' Relationship Timeline",
        "link": "https://www.usmagazine.com/celebrity-news/pictures/olivia-wilde-and-harry-styles-relationship-timeline/",
        "snippet": "Olivia Wilde started dating Harry Styles after ending her years-long engagement to Jason Sudeikis.",
    },
    "people_also_ask": [
        {
            "question": "Who is Harry Styles married to?",
            "answer": "Harry Styles is not married.",
            "link": "https://people.com/music/harry-styles-wife/",
        },
        {
            "question": "Who is Olivia Wilde's current boyfriend?",
            "answer": "Olivia Wilde is reportedly dating again after her split from Harry Styles.",
            "link": "https://www.yahoo.com/now/olivia-wilde-quietly-dating-again-183844364.html",
        },
    ],
}

HTTPX_PATH = "haystack_integrations.components.websearch.serpbase.websearch.httpx"


@pytest.fixture
def mock_serpbase_search_result() -> Generator[MagicMock, None, None]:
    with patch(HTTPX_PATH) as mock_run:
        mock_run.post.return_value = Mock(status_code=200, json=lambda: EXAMPLE_SERPBASE_RESPONSE)
        yield mock_run


@pytest.fixture
def mock_serpbase_search_result_async() -> Generator[MagicMock, None, None]:
    with patch(f"{HTTPX_PATH}.AsyncClient") as mock_run:
        mock_client = AsyncMock()
        mock_client.post.return_value = Mock(status_code=200, json=lambda: EXAMPLE_SERPBASE_RESPONSE)
...
@pytest.fixture
def mock_serpbase_search_result_no_snippet() -> Generator[MagicMock, None, None]:
    resp = copy.deepcopy(EXAMPLE_SERPBASE_RESPONSE)
...
@pytest.fixture
def mock_serpbase_search_result_no_snippet_async() -> Generator[MagicMock, None, None]:
    resp = copy.deepcopy(EXAMPLE_SERPBASE_RESPONSE)
    del resp["organic"][0]["snippet"]
    with patch(f"{HTTPX_PATH}.AsyncClient") as mock_run:
        mock_client = AsyncMock()
...
class TestSerpBaseSearchAPI:
    ...
    def test_to_dict(self, monkeypatch):
        monkeypatch.setenv("SERPBASE_API_KEY", "test-api-key")
        component = SerpBaseWebSearch(top_k=10, allowed_domains=["test.com"], search_params={"param": "test"})
        data = component.to_dict()
        assert data == {
            "type": ("haystack_integrations.components.websearch.serpbase.websearch.SerpBaseWebSearch"),
            "init_parameters": {
                "api_key": {"env_vars": ["SERPBASE_API_KEY"], "strict": True, "type": "env_var"},
                "top_k": 10,
    ...
    def test_from_dict(self, monkeypatch):
        ...
        component = SerpBaseWebSearch.from_dict(data)
        assert component.api_key == Secret.from_env_var("SERPBASE_API_KEY")
    ...
    @pytest.mark.usefixtures("mock_serpbase_search_result")
    def test_web_search_top_k(self, top_k: int) -> None:
        ws = SerpBaseWebSearch(api_key=Secret.from_token("test-api-key"), top_k=top_k)
    ...
    @pytest.mark.parametrize("top_k", [1, 5, 7])
    @pytest.mark.asyncio
    @pytest.mark.usefixtures("mock_serpbase_search_result_async")
    ...
    @pytest.mark.usefixtures("mock_serpbase_search_result")
    def test_featured_snippet_and_people_also_ask(self) -> None:
        ws = SerpBaseWebSearch(api_key=Secret.from_token("test-api-key"), top_k=20)
        results = ws.run(query="Who is the boyfriend of Olivia Wilde?")
        documents = results["documents"]
        # documents include featured snippet + organic + people_also_ask
        assert len(documents) == 1 + 9 + 2
        featured = [d for d in documents if d.meta.get("featured")]
        assert len(featured) == 1
        assert featured[0].meta["link"] == EXAMPLE_SERPBASE_RESPONSE["featured_snippet"]["link"]

    @pytest.mark.usefixtures("mock_serpbase_search_result_no_snippet")
    def test_no_snippet(self) -> None:
        ws = SerpBaseWebSearch(api_key=Secret.from_token("test-api-key"), top_k=1)
        ws.run(query="Who is the boyfriend of Olivia Wilde?")

    @pytest.mark.asyncio
    @pytest.mark.usefixtures("mock_serpbase_search_result_no_snippet_async")
    ...
    def test_timeout_error(self, mock_post: MagicMock, timeout_exception: type[Exception]) -> None:
        ...
        ws = SerpBaseWebSearch(api_key=Secret.from_token("test-api-key"))
    ...
    def test_request_exception(self, mock_post: MagicMock) -> None:
        ...
        with pytest.raises(SerpBaseError):
            ws.run(query="Who is the boyfriend of Olivia Wilde?")
    ...
    @pytest.mark.skipif(
        not os.environ.get("SERPBASE_API_KEY", None),
        reason="Export an env var called SERPBASE_API_KEY containing the SerpBase API key to run this test.",
    )
    @pytest.mark.integration
    def test_web_search(self) -> None:
        ws = SerpBaseWebSearch(top_k=10)
        results = ws.run(query="Who is the boyfriend of Olivia Wilde?")
        documents = results["documents"]
        links = results["links"]
        # documents can also include featured snippet and "people also ask" results, so only links
    ...
    def test_is_domain_allowed_helper_method(self, monkeypatch):
        ...
        ws_no_filter = SerpBaseWebSearch(api_key=Secret.from_token("test-api-key"), allowed_domains=None)
