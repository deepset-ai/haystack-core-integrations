...
def test_factory_behavior(mock_instructor):  # noqa: ARG001
    ...
    same_embedding_backend = _InstructorEmbeddingBackendFactory.get_embedding_backend(
        "hkunlp/instructor-large", "cpu"
    )
    another_embedding_backend = (
...
def test_embedding_function_with_kwargs(mock_instructor):  # noqa: ARG001
    ...
    embedding_backend.model.encode.assert_called_once_with(
        data, normalize_embeddings=True
    )
    # restore the factory state
    _InstructorEmbeddingBackendFactory._instances = {}
