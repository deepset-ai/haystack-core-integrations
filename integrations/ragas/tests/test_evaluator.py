...
def test_initialization_with_multiple_metrics():
...
def test_invalid_metrics_raises_type_error():
...
def test_invalid_metrics_mixed_raises_type_error():
    """Even one non-SimpleBaseMetric in the list should fail."""
...
def test_run_returns_metric_results_keyed_by_name():
...
def test_run_scores_all_metrics():
...
def test_run_calls_score_on_each_metric():
...
# --- run() — parameter filtering ---

def test_score_metric_passes_only_matching_params():
...
def test_score_metric_omits_none_fields():
    """None fields are not forwarded even if they match the signature."""
    metric = _make_metric("faithfulness")
    evaluator = RagasEvaluator(ragas_metrics=[metric])
...
def test_run_accepts_document_objects():
    metric = _make_metric("faithfulness")
    evaluator = RagasEvaluator(ragas_metrics=[metric])
...
def test_run_accepts_string_documents():
    metric = _make_metric("faithfulness")
    evaluator = RagasEvaluator(ragas_metrics=[metric])
...
def test_run_raises_on_invalid_input_types(invalid_input, field_name, error_message):
