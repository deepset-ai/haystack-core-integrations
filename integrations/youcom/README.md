...
`YouComWebSearch` runs without any configuration: with no API key available it uses the You.com keyless
free tier (rate limited per IP). Set the `YDC_API_KEY` environment variable to use the keyed
...
search that needs no key; to also run the keyed tests, export the `YDC_API_KEY` environment variable.
