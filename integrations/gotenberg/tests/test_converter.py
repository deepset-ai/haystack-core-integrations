...
class TestInitialization:
    def test_init_and_serialization_round_trip(self) -> None:
    ...
    def test_init_rejects_invalid_url(self, url: str) -> None:
        with pytest.raises(ValueError, match=r"valid HTTP\(S\)"):
            GotenbergFileConverter(url=url)

    @pytest.mark.parametrize("timeout", [0, -1])
    def test_init_rejects_invalid_timeout(self, timeout: float) -> None:
        with pytest.raises(ValueError, match="greater than zero"):
            GotenbergFileConverter(timeout=timeout)


class TestInputValidation:
    def test_run_rejects_empty_sources(self) -> None:
        with pytest.raises(ValueError, match="at least one"):
            GotenbergFileConverter().run(sources=[])

    @pytest.mark.asyncio
    async def test_run_async_rejects_empty_sources(self) -> None:
        with pytest.raises(ValueError, match="at least one"):
...
class TestURLSources:
    ...
    def test_url_posts_multipart_scalar(self, mock_httpx2_clients, url: str) -> None:
    ...
    def test_malformed_http_urls_are_rejected(self, url: str) -> None:
    ...
    def test_unsupported_url_schemes_are_rejected(self, url: str) -> None:
        with pytest.raises(ValueError, match="Unsupported URL scheme"):
            GotenbergFileConverter().run(sources=[url])


class TestLocalFileSources:
    @pytest.mark.parametrize("source_type", ["str", "path"])
    def test_libreoffice_local_upload_preserves_filename(
        self, tmp_path: Path, mock_httpx2_clients, source_type: str
    ) -> None:
    ...
    def test_supported_libreoffice_suffixes_use_direct_endpoint(
        self, tmp_path: Path, mock_httpx2_clients, suffix: str
    ) -> None:
    ...
    def test_local_html_uploads_index_and_resources(self, tmp_path: Path, mock_httpx2_clients, suffix: str) -> None:
    ...
    def test_colon_in_local_string_path_is_not_treated_as_url(self, tmp_path: Path, mock_httpx2_clients) -> None:
    ...
    def test_invalid_local_suffix_is_rejected(self, tmp_path: Path, filename: str, error: str) -> None:
...
class TestByteStreamSources:
    @pytest.mark.parametrize("mime_type", [None, "", "   ", " ; charset=UTF-8"])
    def test_bytestream_requires_non_empty_mime_type(self, mime_type: str | None) -> None:
    ...
    def test_html_mime_aliases_upload_index_html(self, mock_httpx2_clients, mime_type: str) -> None:
    ...
    def test_markdown_mime_aliases_upload_template_and_source(self, mock_httpx2_clients, mime_type: str) -> None:
    ...
    def test_office_bytestream_derives_valid_upload_extension(
        self, mock_httpx2_clients, mime_type: str, expected_suffix: str
    ) -> None:
    ...
    def test_unsupported_bytestream_mime_type_is_rejected(self) -> None:
    ...
    def test_mime_extension_overrides_metadata_suffix_and_preserves_metadata(self, mock_httpx2_clients) -> None:
        source = ByteStream(
            data=b"actual docx",
    ...
    def test_invalid_utf8_is_rejected_for_text_routes(self, mime_type: str, kind: str) -> None:
...
class TestBatchAndMetadata:
    def test_mixed_batch_posts_in_input_order_and_keeps_pdf_order(self, tmp_path: Path, mock_httpx2_clients) -> None:
    ...
    def test_shared_and_explicit_metadata_behavior(self, mock_httpx2_clients) -> None:
    ...
    def test_metadata_list_length_must_match_sources(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_async_metadata_list_length_must_match_sources(self) -> None:
        with pytest.raises(ValueError, match="length of the metadata list"):
...
class TestAsyncRun:
    @pytest.mark.asyncio
    async def test_run_async_uses_async_client_and_preserves_order(self, mock_httpx2_clients) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_async_posts_requests_concurrently(self, mock_httpx2_clients) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_async_limits_in_flight_requests(self, mock_httpx2_clients) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_async_routes_mixed_sources(self, tmp_path: Path, mock_httpx2_clients) -> None:
        office = tmp_path / "report.docx"
        office.write_bytes(b"docx")
...
class TestResources:
    def test_resources_only_appear_in_html_and_markdown_requests(self, tmp_path: Path, mock_httpx2_clients) -> None:
        resource = tmp_path / "styles.css"
    ...
    def test_resource_validation(self, tmp_path: Path) -> None:
    ...
    def test_markdown_rejects_resource_conflicting_with_source_name(self, tmp_path: Path) -> None:
...
class TestResponseValidation:
    def test_http_error_is_raised(self, mock_httpx2_clients) -> None:
    ...
    def test_zip_content_type_is_rejected(self, mock_httpx2_clients, content_type: str) -> None:
...
class TestIntegration:
    ...
    def test_libreoffice_converts_local_file_routed_by_suffix(self) -> None:
        source = Path("tests/test_files/docx/sample_docx.docx")
        result = GotenbergFileConverter().run(sources=[source])
        _assert_integration_pdf(result=result)

    @pytest.mark.integration
    def test_html_converts_document(self) -> None:
        source = ByteStream(
    ...
    @pytest.mark.integration
    def test_markdown_converts_document(self) -> None:
        source = ByteStream(
    ...
    @pytest.mark.integration
    def test_url_converts_page_without_external_network(self) -> None:
