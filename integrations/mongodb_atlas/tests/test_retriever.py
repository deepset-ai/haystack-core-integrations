...
class TestRetriever:
    ...
    def mock_client(self):
        with patch(
            ...
            mock_collection_names = MagicMock(
                return_value=["test_embeddings_collection"]
            )
    ...
    def test_init_default(self):
        ...
        with pytest.raises(ValueError):
            MongoDBAtlasEmbeddingRetriever(
                document_store=mock_store, filter_policy="wrong_policy"
            )
    ...
    def test_from_dict_no_filter_policy(
        self, monkeypatch
    ):  # mock_client appears unused but is required
        monkeypatch.setenv("MONGO_CONNECTION_STRING", "test_conn_str")
