    ...
    message.meta.update(
        {
            ...
            "usage": {
                "prompt_tokens": (
                    chat_response.usage.billed_units.input_tokens if hasattr(chat_response, "usage") else -1
                ),
                "completion_tokens": (
                    chat_response.usage.billed_units.output_tokens if hasattr(chat_response, "usage") else -1
                ),
            },
            "index": 0,
...
class CohereChatGenerator:
    """
    Completes chats using Cohere's models through Cohere cohere.ClientV2 `chat` endpoint.
