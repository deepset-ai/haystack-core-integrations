...
class TestQdrantDocumentStore(
    ...
    def test_init_is_lazy(self):
        with patch(
            "haystack_integrations.document_stores.qdrant.document_store.qdrant_client"
        ) as mocked_qdrant:
    ...
    def test_query_hybrid(self, generate_sparse_embedding):
        document_store = QdrantDocumentStore(
            location=":memory:", use_sparse_embeddings=True
        )
    ...
    def test_query_hybrid_search_batch_failure(self):
        ...
        with patch.object(
            ...
            "search_batch",
            side_effect=Exception("search_batch error"),
        ):
            with pytest.raises(QdrantStoreError):
                document_store._query_hybrid(
    ...
    @patch(
        "haystack_integrations.document_stores.qdrant.document_store.reciprocal_rank_fusion"
    )
