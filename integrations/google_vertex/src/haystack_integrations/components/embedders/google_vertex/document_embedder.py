...
SUPPORTED_EMBEDDING_MODELS = ["text-embedding-004",
                       "text-embedding-005",
...
class VertexAIDocumentEmbedder:
    ...
    def __init__(
        ...
        batch_size: int = 32,
        max_tokens_total:int= 20000 , # https://cloud.google.com/vertex-ai/generative-ai/docs/embeddings/get-text-embeddings#get_text_embeddings_for_a_snippet_of_text
    ...
    def run(self, documents: List[Document]):
        ...
        while i< len(documents):
            ...
            while total_tokens > self.max_tokens_total:
                ...
                logger.debug(f"Batch {batch_number} Reducing batch size to {batch_size} due to token limit (total_tokens = {total_tokens})")
                batch = documents[i:i+batch_size]
                batch_text = self._prepare_texts_to_embed(batch)
                total_tokens = self.embedder.count_tokens(batch_text).total_tokens
            ...
            while retries > 0:
                try:
                    ...
                    logger.debug(f"Batch {batch_number} Embedding by smaller batchers (batch_size reduced to {math.ceil(batch_size/2)})")

                    try :
                        all_embeddings.extend(self.embed_batch_by_smaller_batches(batch, subbatch=math.ceil(batch_size/2)))
                        break

                    except Exception as e:
                        logger.info(f"{retries}/3: Maximum 500 quota achieved, waiting {self.time_sleep} seconds before retrying")
                        logger.info(e)
                        retries -= 1
                        if retries ==0:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "VertexAITDocumentEmbedder":
        """
