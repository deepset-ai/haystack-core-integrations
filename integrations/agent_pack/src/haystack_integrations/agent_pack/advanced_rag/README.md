...
| `fetch_documents_by_filter` | `FetchDocumentsByFilterTool` | Fetches documents directly by metadata filter, without relevance scoring — for grabbing specific documents (e.g. a known title or file). Results are put into reading order (grouped by parent file, sorted by split/page), `max_docs` per call; larger match sets are paged via the tool's `offset` input. Over-broad filters are refused *before* fetching, as an error the LLM recovers from by narrowing. |
