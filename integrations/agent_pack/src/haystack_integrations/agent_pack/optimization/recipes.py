...
def _serialized_agent(reference: Agent) -> dict[str, Any]:
    """Return an independent serialized copy of an Agent that can be patched."""
    try:
...
def _named_element(elements: list[Any], name: str) -> dict[str, Any] | None:
    """Find a named serialized component or tool inside a list."""
...
def _resolve_patch_target(data: dict[str, Any], path: str) -> tuple[dict[str, Any], str]:
    """Resolve a dotted patch path to the dictionary containing its final key."""
    segments = path.split(".")
    current: Any = data
    for index, segment in enumerate(segments[:-1]):
        if isinstance(current, list):
            if (current := _named_element(elements=current, name=segment)) is None:
...
def _patched_agent(reference: Agent, patch: dict[str, Any]) -> Agent:
    """Rebuild an Agent after applying a patch to its serialized initialization parameters."""
    data = _serialized_agent(reference=reference)
    parameters = data["init_parameters"]
    for path, value in patch.items():
        container, key = _resolve_patch_target(data=parameters, path=path)
        container[key] = value
    try:
        return type(reference).from_dict(data=data)
    except Exception as error:
...
class ModelSubstitutionRecipe(_Recipe):
    ...
    def identity(self, assets: ApprovedAssetCatalog) -> dict[str, Any]:
        ...
        return {**self.model_dump(), "asset": assets.model(model_id=self.model_id).identity()}
...
class ApplyPatchRecipe(_Recipe):
    ...
    def materialize(self, reference: Agent, assets: ApprovedAssetCatalog) -> Agent:
        ...
        return _patched_agent(reference=reference, patch=assets.patch(name=self.patch).patch)

    def identity(self, assets: ApprovedAssetCatalog) -> dict[str, Any]:
        """Include the complete patch definition in this recipe's measurement identity."""
        return {**self.model_dump(), "asset": assets.patch(name=self.patch).identity()}


CandidateRecipe: TypeAlias = Annotated[
    ModelSubstitutionRecipe | ApplyPatchRecipe,
...
def proposal_json_schema(assets: ApprovedAssetCatalog) -> dict[str, Any]:
    ...
    for name in ("ModelSubstitutionRecipe", "ApplyPatchRecipe"):
...
def parse_proposal(payload: dict[str, Any], assets: ApprovedAssetCatalog) -> CandidateRecipe | None:
    ...
    if raw_recipe is not None and (not isinstance(raw_recipe, dict) or "kind" not in raw_recipe):
        ...
        raise ValueError(msg)
    recipe = OptimizerProposal.model_validate(obj=payload).recipe
    if isinstance(recipe, ModelSubstitutionRecipe):
        assets.model(model_id=recipe.model_id)
    elif isinstance(recipe, ApplyPatchRecipe):
        assets.patch(name=recipe.patch)
    return recipe
...
def recipe_fingerprint(recipe: _Recipe, assets: ApprovedAssetCatalog) -> str:
    ...
    payload = json.dumps(recipe.identity(assets=assets), sort_keys=True, default=str)
...
def is_obvious_noop(recipe: _Recipe, reference: Agent) -> bool:
    ...
    if isinstance(recipe, ModelSubstitutionRecipe):
        return recipe.model_id == generator_model_id(generator=reference.chat_generator)
    return False


RECIPE_KINDS = ("model_substitution", "apply_patch")
