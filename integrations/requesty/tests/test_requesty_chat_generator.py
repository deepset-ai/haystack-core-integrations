...
from haystack_integrations.components.generators.requesty.chat.chat_generator import (
    RequestyChatGenerator,
    _convert_requesty_completion_to_chat_message,
    _extract_reasoning,
)
...
class TestRequestyChatGenerator:
    def test_init_default(self, monkeypatch):
        monkeypatch.setenv("REQUESTY_API_KEY", "test-api-key")
        component = RequestyChatGenerator()
        assert component.api_key.resolve_value() == "test-api-key"
        assert component.model == "openai/gpt-4o-mini"
        assert component.api_base_url == "https://router.requesty.ai/v1"
    ...
    def test_to_dict_default(self, monkeypatch):
        ...
        assert (
            data["type"]
            == "haystack_integrations.components.generators.requesty.chat.chat_generator.RequestyChatGenerator"
        )

        expected_params = {
            "api_key": {"env_vars": ["REQUESTY_API_KEY"], "strict": True, "type": "env_var"},
            "model": "openai/gpt-4o-mini",
    ...
    def test_from_dict(self, monkeypatch):
        monkeypatch.setenv("REQUESTY_API_KEY", "fake-api-key")
        data = {
            "type": ("haystack_integrations.components.generators.requesty.chat.chat_generator.RequestyChatGenerator"),
            "init_parameters": {
                "api_key": {"env_vars": ["REQUESTY_API_KEY"], "strict": True, "type": "env_var"},
                "model": "openai/gpt-4o-mini",
                "api_base_url": "test-base-url",
        ...
        component = RequestyChatGenerator.from_dict(data)
        assert component.model == "openai/gpt-4o-mini"
        assert component.streaming_callback is print_streaming_chunk
        assert component.api_base_url == "test-base-url"
        assert component.generation_kwargs == {"max_tokens": 10, "some_test_param": "test-params"}
        assert component.api_key == Secret.from_env_var("REQUESTY_API_KEY")
    ...
    def test_run_with_params(self, chat_messages, mock_chat_completion, monkeypatch):
        monkeypatch.setenv("REQUESTY_API_KEY", "fake-api-key")
        component = RequestyChatGenerator(generation_kwargs={"max_tokens": 10, "temperature": 0.5})
        response = component.run(chat_messages)

        # check that the component calls the OpenAI API with the correct parameters
        # for Requesty, these are passed in the extra_body parameter
    ...
    def test_prepare_api_call_with_tools_strict(self, chat_messages, tools, monkeypatch):
        monkeypatch.setenv("REQUESTY_API_KEY", "fake-api-key")
        component = RequestyChatGenerator(tools=tools)
    ...
    def test_prepare_api_call_raises_when_streaming_with_multiple_responses(self, chat_messages, monkeypatch):
        monkeypatch.setenv("REQUESTY_API_KEY", "fake-api-key")
        component = RequestyChatGenerator(generation_kwargs={"n": 2})
    ...
    def test_prepare_api_call_with_response_format_and_streaming(self, chat_messages, monkeypatch):
        ...
        component = RequestyChatGenerator(generation_kwargs={"response_format": response_format})
    ...
    @pytest.mark.skipif(
        not os.environ.get("REQUESTY_API_KEY", None),
        reason="Export an env var called REQUESTY_API_KEY containing the Requesty API key to run this test.",
    )
    ...
    def test_live_run_wrong_model(self, chat_messages):
        component = RequestyChatGenerator(model="something-obviously-wrong")
    ...
    def test_live_run_streaming(self):
        ...
        callback = Callback()
        component = RequestyChatGenerator(streaming_callback=callback)
    ...
    def test_live_run_with_tools_and_response(self, tools):
        """
        Integration test that the RequestyChatGenerator component can run with tools and get a response.
        """
    ...
    def test_live_run_with_tools_streaming(self, tools):
        """
        Integration test that the RequestyChatGenerator component can run with tools and streaming.
        """
        component = RequestyChatGenerator(tools=tools, streaming_callback=print_streaming_chunk)
    ...
    def test_pipeline_with_requesty_chat_generator(self, tools):
        """
        Test that the RequestyChatGenerator component can be used in a pipeline
        """
        pipeline = Pipeline()
        pipeline.add_component("generator", RequestyChatGenerator(tools=tools))
    ...
    def test_live_run_with_response_format_json_schema(self):
        ...
        # gpt-4o-mini is very rarely flaky but to harden CI we'll be more explicit
        chat_messages = [
        ...
        comp = RequestyChatGenerator(model="openai/gpt-4o-mini", generation_kwargs={"response_format": response_schema})
    ...
    def test_serde_in_pipeline(self, monkeypatch):
        """
        Test serialization/deserialization of RequestyChatGenerator in a Pipeline,
        ...
        generator = RequestyChatGenerator(
        ...
        expected_dict = {
            ...
            "components": {
                "generator": {
                    "type": "haystack_integrations.components.generators.requesty.chat.chat_generator.RequestyChatGenerator",  # noqa: E501
                    "init_parameters": {
                        "api_key": {"type": "env_var", "env_vars": ["REQUESTY_API_KEY"], "strict": True},
                        "model": "openai/gpt-4o-mini",
                        "streaming_callback": "haystack.components.generators.utils.print_streaming_chunk",
                        "api_base_url": "https://router.requesty.ai/v1",
                        "generation_kwargs": {"temperature": 0.7},
    ...
    def test_live_run_with_response_format_pydantic_model(self, calendar_event_model):
        ...
        component = RequestyChatGenerator(
            model="openai/gpt-4o-mini", generation_kwargs={"response_format": calendar_event_model}
        )
        results = component.run(chat_messages)
        assert len(results["replies"]) == 1
        message: ChatMessage = results["replies"][0]
...
class TestChatCompletionChunkConversion:
    def test_handle_stream_response(self):
        ...
        llm = RequestyChatGenerator(api_key=Secret.from_token("test-api-key"))
        result = llm._handle_stream_response(requesty_chunks, callback=collector_callback)[0]  # type: ignore
...
class TestReasoningSupport:
    ...
    def test_convert_completion_with_reasoning_and_tool_calls(self):
        ...
        result = _convert_requesty_completion_to_chat_message(completion, completion.choices[0])
        assert result.text is None
