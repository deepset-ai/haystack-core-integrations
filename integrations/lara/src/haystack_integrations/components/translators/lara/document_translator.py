...
class LaraDocumentTranslator:
    ...
    def run(self,
        ...
        :param adapt_to:
            ...
            of these memories at inference time. Domain adaptation is available on Team and Enterprise plans. You can find more 
            detailed information in the 
            [Lara documentation](https://developers.laratranslate.com/docs/adapt-to-translation-memories).
        :param glossaries:
            Optional list of glossary IDs. Lara applies these glossaries at inference time to enforce 
    ...
    def _validate_params(
        ...
        if isinstance(source_lang, list) and len(source_lang) != num_documents:
            raise ValueError("If source language is a list, it must be the same length as the number of documents.")
        validated_params["source_lang"] = [source_lang] * num_documents if not isinstance(source_lang, list) else source_lang

        if isinstance(target_lang, list) and len(target_lang) != num_documents:
            raise ValueError("If target language is a list, it must be the same length as the number of documents.")
        validated_params["target_lang"] = [target_lang] * num_documents if not isinstance(target_lang, list) else target_lang

        if isinstance(context, list) and len(context) != num_documents:
            raise ValueError("If context is a list, it must be the same length as the number of documents.")
        ...
        if isinstance(style, list) and len(style) != num_documents:
            raise ValueError("If style is a list, it must be the same length as the number of documents.")
        ...
        if isinstance(instructions, list) and len(instructions) != num_documents:
            raise ValueError("If instructions is a list, it must be the same length as the number of documents.")
        if instructions is not None:
            validated_params["instructions"] = [[instructions]] * num_documents if not isinstance(instructions, list) else instructions
        else:
            validated_params["instructions"] = [None] * num_documents
        
        if isinstance(reasoning, list) and len(reasoning) != num_documents:
            raise ValueError("If reasoning is a list, it must be the same length as the number of documents.")
        ...
        if (isinstance(adapt_to, list) 
            ...
            raise ValueError("If adapt to is a list of lists, it must be the same length as the number of documents.")
        validated_params["adapt_to"] = [adapt_to] * num_documents if not (isinstance(adapt_to, list) and len(adapt_to) > 0 and isinstance(adapt_to[0], list)) else adapt_to
        ...
        if (
            ...
            raise ValueError("If glossaries is a list of lists, it must be the same length as the number of documents.")
        validated_params["glossaries"] = [glossaries] * num_documents if not (isinstance(glossaries, list) and len(glossaries) > 0 and isinstance(glossaries[0], list)) else glossaries
