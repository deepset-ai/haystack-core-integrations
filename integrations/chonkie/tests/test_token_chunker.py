...
class TestChonkieTokenDocumentSplitter:
    ...
    def test_to_dict(self):
        ...
        assert data == {
            "type": "haystack_integrations.components.preprocessors.chonkie.token_chunker.ChonkieTokenDocumentSplitter",
            "init_parameters": {
                "chunk_size": 1024,
                "tokenizer": "word",
                "chunk_overlap": 50,
    ...
    def test_run_none_content(self):
        chunker = ChonkieTokenDocumentSplitter()
        with pytest.raises(ValueError, match=r"ChonkieTokenDocumentSplitter works only with text documents but doc ID .* is None"):
