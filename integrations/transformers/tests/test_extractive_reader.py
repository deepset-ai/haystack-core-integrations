...
def mock_tokenizer():
    ...
    with patch(
        "haystack_integrations.components.readers.transformers.extractive_reader.AutoTokenizer.from_pretrained"
    ) as tokenizer:
...
def mock_reader(mock_tokenizer):
    ...
    with patch(
        ...
        reader = TransformersExtractiveReader(model="mock-model", device=ComponentDevice.from_str("cpu"))
        reader.warm_up()
        return reader
...
def test_to_dict(initialized_token: Secret):
    component = TransformersExtractiveReader(
        "my-model", token=initialized_token, model_kwargs={"torch_dtype": torch.float16}
    )
    data = component.to_dict()

    assert data == {
        "type": (
            "haystack_integrations.components.readers.transformers.extractive_reader.TransformersExtractiveReader"
        ),
        "init_parameters": {
            "model": "my-model",
            "device": None,
...
def test_to_dict_empty_model_kwargs(initialized_token: Secret):
    component = TransformersExtractiveReader("my-model", token=initialized_token)
    data = component.to_dict()
...
def test_to_dict_device_map(device_map, expected):
    component = TransformersExtractiveReader("my-model", model_kwargs={"device_map": device_map})
    data = component.to_dict()
...
def test_from_dict():
    ...
    component = TransformersExtractiveReader.from_dict(data)
...
def test_run_no_docs(mock_reader: TransformersExtractiveReader):
    assert mock_reader.run(query="hello", documents=[]) == {"answers": []}


def test_output(mock_reader: TransformersExtractiveReader):
...
def test_flatten_documents(mock_reader: TransformersExtractiveReader):
...
def test_preprocess(mock_reader: TransformersExtractiveReader):
...
def test_preprocess_splitting(mock_reader: TransformersExtractiveReader):
    _, _, seq_ids, _, query_ids, doc_ids = mock_reader._preprocess(
...
def test_postprocess(mock_reader: TransformersExtractiveReader):
...
def test_nest_answers(mock_reader: TransformersExtractiveReader):
    start = list(range(5))
...
def test_postprocess_filters_probs_when_answers_per_seq_exceeds_valid_spans(mock_reader: TransformersExtractiveReader):
...
def test_nest_answers_accepts_variable_length_probability_rows(mock_reader: TransformersExtractiveReader):
...
def test_add_answer_page_number_returns_same_answer(mock_reader: TransformersExtractiveReader, caplog):
...
def test_add_answer_page_number_with_none_meta(mock_reader: TransformersExtractiveReader):
...
def test_add_answer_page_number_with_form_feed(mock_reader: TransformersExtractiveReader):
    document = Document(
...
def test_device_map_and_device_warning(caplog):
    with caplog.at_level(logging.WARNING):
        _ = TransformersExtractiveReader(
...
class TestDeduplication:
    ...
    def test_calculate_overlap(self, mock_reader: TransformersExtractiveReader, doc1: Document):
        answer1 = "the river"
    ...
    def test_should_keep_false(
        self,
        mock_reader: TransformersExtractiveReader,
        doc1: Document,
        doc2: Document,
    ...
    def test_deduplicate_by_overlap_none_overlap(
        self, mock_reader: TransformersExtractiveReader, candidate_answer: ExtractedAnswer
    ):
    ...
    def test_deduplicate_by_overlap(
        self, mock_reader: TransformersExtractiveReader, candidate_answer: ExtractedAnswer, doc1: Document
    ):
        answer2 = "Maine"
