...
from haystack_integrations.document_stores.alloydb.converters import (
...
def test_from_pg_to_haystack_documents():
    ...
    assert haystack_docs[0].embedding == pytest.approx([0.1, 0.2, 0.3])
    ...
    assert haystack_docs[1].embedding == pytest.approx([0.4, 0.5, 0.6])
    ...
    assert haystack_docs[2].embedding == pytest.approx([0.7, 0.8, 0.9])
    assert haystack_docs[2].blob == ByteStream(
        b"test", meta={"blob_meta_key": "blob_meta_value"}, mime_type="mime_type"
    )

    assert haystack_docs[3].id == "4"
    assert haystack_docs[3].content == "This is a text"
    assert haystack_docs[3].meta == {"meta_key": "meta_value"}
    assert haystack_docs[3].embedding is None
    assert haystack_docs[3].score == 0.9
