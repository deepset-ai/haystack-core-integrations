...
class TestEmbeddingRetrieval:
    ...
    def test_embedding_retrieval_cosine_similarity(self, document_store: AlloyDBDocumentStore):
    ...
    @pytest.mark.parametrize("document_store", ["document_store", "document_store_w_hnsw_index"], indirect=True)
    def test_embedding_retrieval_inner_product(self, document_store: AlloyDBDocumentStore):
        query_embedding = [0.1] * 768
        most_similar_embedding = [0.8] * 768
        second_best_embedding = [0.8] * 700 + [0.1] * 3 + [0.2] * 65
        another_embedding = rand(768).tolist()
    ...
    @pytest.mark.parametrize("document_store", ["document_store", "document_store_w_hnsw_index"], indirect=True)
    def test_embedding_retrieval_l2_distance(self, document_store: AlloyDBDocumentStore):
        query_embedding = [0.1] * 768
    ...
    @pytest.mark.parametrize("document_store", ["document_store", "document_store_w_hnsw_index"], indirect=True)
    def test_embedding_retrieval_with_filters(self, document_store: AlloyDBDocumentStore):
    ...
    def test_empty_query_embedding(self, document_store: AlloyDBDocumentStore):
        query_embedding: list[float] = []
        with pytest.raises(ValueError):
            document_store._embedding_retrieval(query_embedding=query_embedding)

    def test_query_embedding_wrong_dimension(self, document_store: AlloyDBDocumentStore):
...
class TestKeywordRetrieval:
    def test_keyword_retrieval(self, document_store: AlloyDBDocumentStore):
        docs = [
    ...
    def test_empty_query(self, document_store: AlloyDBDocumentStore):
        with pytest.raises(ValueError):
