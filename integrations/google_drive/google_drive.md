...
- <code>GoogleDriveConfigError</code> – If `max_retries` is negative.

#### run

```python
run(
    access_token: str | Secret, targets: list[Document] | list[str]
...
- **access_token** (<code>str | Secret</code>) – A delegated Google OAuth bearer token for the user whose files are fetched, typically
  ...
  resolved internally.
- **targets** (<code>list\[Document\] | list\[str\]</code>) – The files to fetch, as either `Document`s emitted by `GoogleDriveRetriever` or raw Google
