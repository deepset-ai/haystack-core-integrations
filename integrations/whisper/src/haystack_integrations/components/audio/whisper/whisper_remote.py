    ...
    def run(self, sources: list[str | Path | ByteStream]) -> dict[str, Any]:
        ...
        for source in sources:
            if isinstance(source, ByteStream):
                ...
                byte_stream = ByteStream.from_file_path(Path(source))
                byte_stream.meta["file_path"] = source

            file = io.BytesIO(byte_stream.data)
            file.name = str(byte_stream.meta["file_path"]) if "file_path" in byte_stream.meta else "__fallback__.wav"

            content = self.client.audio.transcriptions.create(file=file, model=self.model, **self.whisper_params)
            doc = Document(content=content.text, meta=byte_stream.meta)
