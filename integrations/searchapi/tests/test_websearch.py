...
from haystack_integrations.components.websearch.searchapi import SearchApiError, SearchApiWebSearch
...
def mock_searchapi_search_result_async() -> Generator[MagicMock, None, None]:
    with patch("haystack_integrations.components.websearch.searchapi.websearch.httpx.AsyncClient") as mock_get:
...
class TestSearchApiSearchAPI:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("top_k", [1, 5, 7])
    @pytest.mark.usefixtures("mock_searchapi_search_result_async")
