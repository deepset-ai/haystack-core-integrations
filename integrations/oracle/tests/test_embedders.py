...
from haystack_integrations.components.embedders.oracle._base import (
    _execute_with_fetch_lobs,
    _execute_with_fetch_lobs_async,
)
...
def test_text_embedder_run_returns_single_embedding(monkeypatch, connection_config):
...
def test_text_embedder_rejects_non_string(connection_config):
    embedder = OracleTextEmbedder(
        connection_config=connection_config(secret_source="env_var"),
        embedding_params={"provider": "database", "model": "demo"},
    )
...
def test_text_embedder_execute_uses_fetch_lobs_when_supported():
    class FakeCursor:
        def __init__(self):
            self.call = None

        def execute(self, statement, parameters=None, *, fetch_lobs=True):
            self.call = (statement, parameters, fetch_lobs)

    cursor = FakeCursor()

    _execute_with_fetch_lobs(cursor, "SELECT 1 FROM DUAL", ["p0"])

    assert cursor.call == ("SELECT 1 FROM DUAL", ["p0"], False)


@pytest.mark.asyncio
async def test_text_embedder_execute_async_uses_fetch_lobs_when_supported():
    class FakeCursor:
        def __init__(self):
            self.call = None

        async def execute(self, statement, parameters=None, *, fetch_lobs=True):
            self.call = (statement, parameters, fetch_lobs)

    cursor = FakeCursor()

    await _execute_with_fetch_lobs_async(cursor, "SELECT 1 FROM DUAL", ["p0"])

    assert cursor.call == ("SELECT 1 FROM DUAL", ["p0"], False)


@pytest.mark.asyncio
async def test_text_embedder_async_awaits_gettype(monkeypatch, connection_config):
    class FakeLob:
        def __init__(self):
            self.value = None
...
@pytest.mark.asyncio
async def test_text_embedder_async_reads_lob_rows(monkeypatch, connection_config):
    class FakeLob:
        async def write(self, _):
            return None

        async def read(self):
            return '{"embed_vector": "[0.1, 0.2, 0.3]"}'
...
def test_document_embedder_prepares_metadata_and_content(connection_config):
    ...
    assert not isinstance(embedder, OracleTextEmbedder)
...
def test_document_embedder_sets_document_embeddings(monkeypatch, connection_config):
    embedder = OracleDocumentEmbedder(
        connection_config=connection_config(secret_source="env_var"),
        embedding_params={"provider": "database", "model": "demo"},
    )
    ...
    assert result["documents"] is not documents
    assert documents[0].embedding is None
    assert documents[1].embedding is None
    assert result["documents"][0].embedding == [0.4, 0.5]
    assert result["documents"][1].embedding == [0.6, 0.7]


def test_embedder_to_dict_keeps_connection_config_secret_structured(connection_config):
    embedder = OracleTextEmbedder(
        connection_config=connection_config(secret_source="env_var"),
        embedding_params={"provider": "database", "model": "demo"},
    )
