...
class SentenceTransformersDiversityRanker:
    ...
    def __init__(
        ...
        *,
        ...
        :param processor_kwargs:
            ...
            model, forwarded as `processor_kwargs`. For example, `{"model_max_length": 128}` shortens the
    ...
    def _embed_and_normalize(self, query: str, texts_to_embed: list[str]) -> tuple[Any, Any]:
        assert self.model is not None  # noqa: S101  # verified in run but mypy doesn't see it
    ...
    def _check_lambda_threshold(lambda_threshold: float, strategy: DiversityRankingStrategy) -> None:
        if (strategy == DiversityRankingStrategy.MAXIMUM_MARGIN_RELEVANCE) and not 0 <= lambda_threshold <= 1:
            msg = f"lambda_threshold must be between 0 and 1, but got {lambda_threshold}."
            raise ValueError(msg)
