...
"""
MistralAgent using the official Mistral Python SDK.

This implementation uses the `mistralai` package instead of direct HTTP requests,
providing better error handling, automatic retries, and maintained compatibility
...
@component
class MistralAgentSDK:
    """
    Generates text using Mistral AI Agents via the official Mistral Python SDK.

    This component uses the `mistralai` package to interact with pre-configured
    Mistral Agents. It provides better error handling and automatic retries
    ...
    Usage example:
    ```python
    from haystack_integrations.components.agents.mistral import MistralAgentSDK
    from haystack.dataclasses import ChatMessage

    # Initialize with your agent ID from the Mistral console
    agent = MistralAgentSDK(agent_id="your-agent-id")
    ...
    def __init__(
        ...
        *,
        ...
        """
        Creates an instance of MistralAgentSDK.
        ...
        # Validate tools for duplicates
        _check_duplicate_tool_names(flatten_tools_or_toolsets(self.tools))

        # Client will be initialized in warm_up()
        self._client = None
        self._async_client = None

    def warm_up(self):
        """
        Initialize the Mistral SDK client.

        This is called automatically before the first request.
        """
        ...
        try:
            ...
            raise ImportError(
                "The 'mistralai' package is required for MistralAgentSDK. "
    ...
    def _convert_messages_to_sdk_format(
        self, messages: list[ChatMessage]
    ) -> list[dict[str, Any]]:
        """
        Convert Haystack ChatMessages to Mistral SDK message format.

        The Mistral SDK expects messages as dicts with 'role' and 'content' keys.
        """
        sdk_messages = []

        for msg in messages:
            # Use the OpenAI format which is compatible with Mistral
    ...
    def _build_tools_for_sdk(
        self, tools: Optional[ToolsType] = None
    ) -> Optional[list[dict[str, Any]]]:
        """Convert Haystack tools to Mistral SDK format."""
    ...
    def _parse_sdk_response(self, response: Any) -> list[ChatMessage]:
        """
        Parse the Mistral SDK response into Haystack ChatMessages.
    ...
    def _handle_streaming(
        ...
        :returns: List containing the final assembled ChatMessage
        """
        collected_content = ""
    ...
    def run(
        ...
        sdk_tools = self._build_tools_for_sdk(tools)
        if sdk_tools:
        ...
        try:
            if effective_callback:
                ...
                replies = self._handle_streaming(stream_response, effective_callback)
            else:
                # Non-streaming request
                response = self._client.agents.complete(**request_kwargs)
                replies = self._parse_sdk_response(response)

            return {"replies": replies}

        except Exception as e:
            # Import error types from mistralai
            try:
                from mistralai import models

                if isinstance(e, models.HTTPValidationError):
                    logger.error(
                        "Mistral validation error: {detail}",
                        detail=e.data.detail if hasattr(e, "data") else str(e),
                    )
                    raise ValueError(f"Mistral validation error: {e}") from e
                elif isinstance(e, models.MistralError):
                    logger.error(
                        "Mistral API error: {status_code} - {message}",
                        status_code=e.status_code,
                        message=e.message,
                    )
                    raise ValueError(f"Mistral API error ({e.status_code}): {e.message}") from e
            except ImportError:
                pass
            raise
    ...
    async def run_async(
        ...
        try:
            if effective_callback:
                ...
                replies = await self._handle_async_streaming(stream_response, effective_callback)
            else:
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "MistralAgentSDK":
