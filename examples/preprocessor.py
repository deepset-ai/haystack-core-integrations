...
class PreProcessor(BasePreProcessor):
    ...
    def _long_documents(self, documents: List[Document], max_chars_check=10_000):
        ...
        for document in documents:
            if len(document.text) > max_chars_check:
                logger.warning(
                ...
                document.text = document.text[:max_chars_check]
                fields.pop("id")
    ...
    def clean(
        ...
        headlines = document.metadata["headlines"] if "headlines" in document.metadata else []
        ...
        if text != document.text:
            ...
            document.text = text
        if headlines:
            document.metadata["headlines"] = headlines
    ...
    def _create_docs_from_splits(
        ...
        for i, txt in enumerate(text_splits):
            meta = deepcopy(meta)
            doc = Document(text=txt, metadata=meta)
            doc.metadata["_split_id"] = i
            if self.add_page_number:
                doc.metadata["page"] = splits_pages[i]
            if headlines:
                ...
                doc.metadata["headlines"] = relevant_headlines
            if split_overlap > 0:
                doc.metadata["_split_overlap"] = []
                if i != 0:
    ...
    def _add_split_overlap_information(
        ...
        """
        overlapping_range = (current_doc_start_idx - previos_doc_start_idx, len(previous_doc.text) - 1)
        if overlapping_range[0] < overlapping_range[1]:
            overlapping_str = previous_doc.text[overlapping_range[0] : overlapping_range[1]]
            if current_doc.text.startswith(overlapping_str):
                # Add split overlap information to previous Document regarding this Document
                previous_doc.metadata["_split_overlap"].append({"doc_id": current_doc.id, "range": overlapping_range})
                # Add split overlap information to this Document regarding the previous Document
                overlapping_range = (0, overlapping_range[1] - overlapping_range[0])
                current_doc.metadata["_split_overlap"].append({"doc_id": previous_doc.id, "range": overlapping_range})
