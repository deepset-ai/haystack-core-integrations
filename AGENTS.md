...
- Keep `to_dict()`/`from_dict()` symmetric with `__init__` — every constructor argument that affects runtime behaviour round-trips (retry, batching, metadata options); build on `default_to_dict`/`default_from_dict` and do any custom work around them (serialize a `Secret`, deserialize a nested document store) rather than replacing them; components serialize as `{"type": ..., "init_parameters": {...}}`, and only `Tool`s use the `{"type": ..., "data": {...}}` shape
