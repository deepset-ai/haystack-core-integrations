...
- Keep `to_dict()`/`from_dict()` symmetric with `__init__` — every constructor argument that affects runtime behaviour round-trips (retry, batching, metadata options); build on `default_to_dict`/`default_from_dict` and do any custom work around them (e.g. `deserialize_secrets_inplace` or rebuilding a nested document store before `default_from_dict`) rather than replacing them; components serialize as `{"type": ..., "init_parameters": {...}}`, and only `Tool`s use the `{"type": ..., "data": {...}}` shape
- Keep `__init__` light: set clients and models to `None` there and create them in `warm_up()` — anything slow to load (a torch model) or that opens a network connection, including resolving `Secret`s; guard with `if self.client is None:` so `warm_up()` is idempotent, and validate concrete backend objects in `__init__` so incompatible dependencies fail fast
- Keep public signatures protocol-compatible: preserve protocol parameter order, append store-specific args after it, and make new optional params keyword-only; don't give a `run(...)` input the component needs a `None` default that silently turns the call into a no-op — keeps Document Store APIs consistent across backends so callers can switch without breaking
...
- In docstrings use single backticks for inline code, single-line Markdown links, and `### Usage example` with fenced ` ```python ` blocks — avoids renderer issues
